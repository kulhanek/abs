...
bool CResourceValue::TestVMem(std::ostream& sout,bool& rstatus)
{
    if( Value == "mem" ){
        // ok - vmem will be the same as mem token
        return(true);
    }
    ...
    if( svalue.find_first_not_of(legall_characters) != string::npos ){
        if( rstatus == true ) sout << endl;
        sout << "<b><red> ERROR: Illegal virtual memory token (vmem) specification!" << endl;
        sout <<         "        Only integer number and one of b,kb,mb,gb,tb sufixes is allowed!</red></b>" << endl;
        rstatus = false;
        return(false);
    }
    ...
    if( (munit != "b")  &&
        ...
        (munit != "tb") ) {
        if( rstatus == true ) sout << endl;
        sout << "<b><red> ERROR: Illegal suffix for virtual memory token (vmem)!" << endl;
        sout <<         "        Allowed suffixes are: b,kb,mb,gb,tb.</red></b>" << endl;
        rstatus = false;
        return(false);
    }
...
bool CResourceValue::TestScratch(std::ostream& sout,bool& rstatus)
{
    string svalue(Value);
    ...
    if( svalue.find_first_not_of(legall_characters) != string::npos ){
        if( rstatus == true ) sout << endl;
        sout << "<b><red> ERROR: Illegal scratch size token (scratch) specification!" << endl;
        sout <<         "        Only integer number and one of b,kb,mb,gb,tb sufixes is allowed!</red></b>" << endl;
        rstatus = false;
        return(false);
    }
    ...
    if( (munit != "b")  &&
        ...
        (munit != "tb") ) {
        if( rstatus == true ) sout << endl;
        sout << "<b><red> ERROR: Illegal suffix for scratch size token (scratch)!" << endl;
        sout <<         "        Allowed suffixes are: b,kb,mb,gb,tb.</red></b>" << endl;
        rstatus = false;
        return(false);
    }
...
bool CResourceValue::TestCPUFreq(std::ostream& sout,bool& rstatus)
{
    string legall_characters = "01234567890";
    string legall_char_short = "0-9";

    string svalue = string(Value);

    if( svalue.find_first_not_of(legall_characters) != string::npos ){
        if( rstatus == true ) sout << endl;
        sout << "<b><red> ERROR: Illegal character in CPU frequency resource token (cpu_freq)!</red></b>" << endl;
...
bool CResourceValue::TestHost(std::ostream& sout,bool& rstatus)
{
...
bool CResourceValue::TestSurrogate(std::ostream& sout,bool& rstatus)
{
    HelperValue = true;
    return(true);
}
