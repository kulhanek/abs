...
#include "PRecycleCPMDJobType.hpp"
...
CComObject* PRecycleCPMDJobTypeCB(void* p_data);

CExtUUID        PRecycleCPMDJobTypeID(
                    "{PRECYCLE_CPMD_JOB_TYPE:0d0f2ef4-c8de-42fb-b52b-58b6b1e41f4a}",
                    "PRecycleCPMD Job Type",
                    "checking of input file for recycle jobs using CPMD");

CPluginObject   PRecycleCPMDJobTypeObject(&BasicJobTypesPlugin,
                    PRecycleCPMDJobTypeID,JOB_TYPE_CAT,
                    PRecycleCPMDJobTypeCB);

// -----------------------------------------------------------------------------

CComObject* PRecycleCPMDJobTypeCB(void* p_data)
{
    CComObject* p_object = new CPRecycleCPMDJobType();
    return(p_object);
}
...
CPRecycleCPMDJobType::CPRecycleCPMDJobType(void)
    : CJobType(&PRecycleCPMDJobTypeObject)
{
}
...
ERetStatus CPRecycleCPMDJobType::DetectJobType(CJob& job,bool& detected,std::ostream& sout)
{
    detected = false;
    ...
    if( PluginDatabase.FindObjectConfigValue(PRecycleCPMDJobTypeID,"_type",job_type) == false ){
    ...
    sout << endl;
    sout << "# ------------------------------------------------" << endl;
    sout << "# precycleCPMD job summary" << endl;
    sout << "# ------------------------------------------------" << endl;
    ...
    //--------------------------------------------------------------------------
    CSmallString psd_name = GetInputFileKeyValue("PRECYCLE_PSEUDOPOTENTIALS");
    if( psd_name == NULL ){
        sout << endl;
        sout << "<b><red> ERROR: The directory with pseudopotentials (PRECYCLE_PSEUDOPOTENTIALS) is not provided!</red></b>" << endl;
        return(ERS_FAILED);
    }
    if( psd_name == NULL ){
    sout << "# Pseudopotentials    : -job working directory-" << endl;
    } else {
    sout << "# Pseudopotentials    : " << psd_name << endl;
    if( CFileSystem::IsDirectory(psd_name) == false ) {
        sout << endl;
        sout << "<b><red> ERROR: The specified directory with pseudopotentials (PRECYCLE_PSEUDOPOTENTIALS) does not exist!</red></b>" << endl;
        return(ERS_FAILED);
    }
    }
    ...
    for(int i=stop+1; i >= start; i--){
        ...
        test_file_form = storage / name_format + ".RESTART";
    ...
    if( current > stop ){
        sout << endl;
        sout << "<b><blue> INFO: The precycleCPMD job processed all stages!</blue></b>" << endl;
    ...
    job.SetItem("basic/jobinput","INF_JOB_CHECK",PRecycleCPMDJobTypeID.GetFullStringForm());
...
bool CPRecycleCPMDJobType::CheckInputFile(CJob& job,std::ostream& sout)
{
...
bool CPRecycleCPMDJobType::CacheJobFile(const CSmallString& file)
{
    ifstream ifs;
    ifs.open(file);
    if( ! ifs ) {
...
const CSmallString CPRecycleCPMDJobType::GetInputFileKeyValue(const CSmallString& key)
{
